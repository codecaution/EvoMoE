...
from fairseq.modules.moe import TopKGate
from fairseq.modules.moe.topkgate import topkgating
...
def test_create_cuda():
    gate = TopNGate(4, 8).cuda()


def do_test_forward(device):
    torch.manual_seed(3)
    input = torch.randn(12, 4).to(device)
    gate = TopKGate(4, 6).to(device)
    capacity = 12
    l_aux, combine_weights, dispatch_mask, _ = gate(input)
    # assert pytest.approx(l_aux.item(), 0.0283)
    assert combine_weights.shape == (12, 6, capacity)
    assert dispatch_mask.shape == (12, 6, capacity)
